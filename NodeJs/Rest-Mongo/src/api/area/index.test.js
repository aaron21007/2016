...
beforeEach(async () => {
  ...
  area = await Area.create({nombre:'hola', actividad:'actividad'})
})

test('POST /areas 201 (admin)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: adminSession, nombre: 'test', actividad: 'test' })
  expect(status).toBe(201)
  expect(typeof body).toEqual('object')
  expect(body.nombre).toEqual('test')
  expect(body.actividad).toEqual('test')
})
